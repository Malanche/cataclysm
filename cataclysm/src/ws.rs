...
impl WebSocketHandshake {
    ...
    pub async fn perform(self, stream: Stream, request: Request) -> Result<WebSocketStream, Error> {
        if request.header.headers.get("Upgrade").map(|u| u.get(0).map(|v| v == "websocket")).flatten().unwrap_or(false) && request.header.headers.get("Connection").map(|c| c.get(0).map(|v| v == "Upgrade" || v == "keep-alive, Upgrade")).flatten().unwrap_or(false) {
            if let Some(nonce) = request.header.headers.get("Sec-WebSocket-Key").map(|wsk| wsk.get(0)).flatten() {
                ...
                if let Some(protocol) = self.protocol {
                    if let Some(available_protocols) = request.header.headers.get("Sec-WebSocket-Protocol") {
                        let mut found = false;
